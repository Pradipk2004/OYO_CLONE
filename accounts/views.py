...
def login_vendor(request):
    if request.method == "POST":
        ...
        try:
            ...
            print(f"[DEBUG] Vendor found: {vendor}, username: {vendor.username}, is_verified: {vendor.is_verified}")
        except HotelVendor.DoesNotExist:
            print("[DEBUG] Vendor not found for email:", email)
            messages.warning(request, "No account found.")
            return redirect('/account/login-vendor/')

        if not vendor.is_verified:
            print("[DEBUG] Vendor account not verified.")
            messages.warning(request, "Account not verified.")
            return redirect('/account/login-vendor/')

        user = authenticate(request, username=vendor.username, password=password)
        print("[DEBUG] Authentication result:", user)

        if user:
            login(request, user)
            print("[DEBUG] Vendor logged in successfully.")
            messages.success(request, "Login successful.")
            return redirect('/account/dashboard/')
        else:
            print("[DEBUG] Invalid credentials for username:", vendor.username)
            messages.warning(request, "Invalid credentials.")
            return redirect('/account/login-vendor/')
...
def register_vendor(request):
    if request.method == "POST":
        ...
        vendor = HotelVendor.objects.create_user(
            username=phone_number, 
            email=email,
            password=password,
            first_name=first_name,
        ...
        vendor.email_token = generateRandomToken()
        ...
        messages.success(request, "An email has been sent to your email address for verification.")
        return redirect('/account/register-vendor/')
...
def dashboard(request):
    hotels = Hotel.objects.filter(hotel_owner=request.user).prefetch_related('hotel_images', 'ameneties')
